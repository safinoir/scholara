...
One paragraph on the results page can be generated by an LLM. It **never selects techniques** — it only rewords the rationale the engine already produced.
...
```
COACH_API_KEY=...                        # any OpenAI-compatible key
COACH_BASE_URL=https://api.openai.com/v1 # or NaviGator, Azure OpenAI, etc.
COACH_MODEL=gpt-4o-mini
```

Safety properties: the key is server-side only, request bodies are Zod-validated, no free-text or personal data is sent, there's an 8-second timeout, and **any** failure (missing key, error, timeout) silently falls back to deterministic pre-written copy. The app is fully functional with no key set.
...
There is no backend, no database, no analytics, and no telemetry. Your profile lives in `localStorage` and can be deleted in one click from `/about`. Shared links encode the persona into the URL itself — no server involved, and the shared payload deliberately excludes your context answers.
